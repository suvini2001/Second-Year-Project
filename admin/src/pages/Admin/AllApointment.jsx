      ...
      <div className='bg-gradient-to-br from-blue-50 via-blue-100 to-cyan-50 border-2 border-blue-300 rounded-2xl shadow-xl text-sm max-h-[80vh] min-h-[60vh] overflow-y-scroll'>
        ...
        {(appointments || []).map((item, index) => (
          ...
          >
            ...
            <p className='max-sm:hidden z-20 text-lg font-medium bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 text-transparent bg-clip-text drop-shadow-md'>{index + 1}</p>

            <div className='flex items-center gap-2 z-20'>
              <img className='w-8 h-8 rounded-full object-cover border-2 border-blue-200 group-hover:border-blue-400 transition shadow' src={item.userData?.image || ''} alt='' />
              <p className='font-semibold text-base bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 text-transparent bg-clip-text drop-shadow'>{item.userData?.name}</p>
            </div>

            <p className='max-sm:hidden z-20 text-base font-light bg-gradient-to-r from-blue-800 via-blue-700 to-blue-600 text-transparent bg-clip-text drop-shadow'>{calculateAge(item.userData?.dob)}</p>

            <p className='z-20 text-base font-medium bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 text-transparent bg-clip-text drop-shadow'>{slotDateFormat(item.slotDate)}, {item.slotTime}</p>

            <div className='flex items-center gap-2 z-20'>
              <img className='w-8 h-8 rounded-full object-cover bg-blue-100 border-2 border-blue-200 group-hover:border-blue-400 transition shadow' src={item.docData?.image || ''} alt='' />
              <p className='font-semibold text-base bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 text-transparent bg-clip-text drop-shadow'>{item.docData?.name}</p>
            </div>

            <p className='font-bold z-20 text-base bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 text-transparent bg-clip-text drop-shadow'>{currency}{item.amount}</p>
            {item.cancelled
              ? <p className='text-red-400 text-xs font-semibold z-20 drop-shadow'>Cancelled</p>
              : item.isCompleted
                ? <p className='text-green-400 text-xs font-semibold z-20 drop-shadow'>Completed</p>
                : <img
                    onClick={() => {
