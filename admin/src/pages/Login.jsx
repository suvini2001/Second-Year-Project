...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (state === "Admin") {
        // Remove doctor token if present
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-transparent">
      <form
        ...
        className={`backdrop-blur-xl shadow-2xl rounded-2xl p-10 min-w-[340px] sm:min-w-96 flex flex-col gap-6 border border-blue-800/60
      ...
      >
        <p className="text-3xl font-extrabold text-white mb-2 text-center tracking-wide drop-shadow-lg">
          <span
            className={state === "Admin" ? "text-blue-400" : "text-blue-400"}
          >
            {state}
          </span>{" "}
          Login
        ...
        <div className="flex justify-between mt-2 text-sm">
          <a
            href="#"
            className="text-white hover:underline transition hover:text-cyan-200"
          >
            Forgot password?
          </a>
          <a
