        ...
        <div className="bg-gradient-to-br from-blue-950 via-blue-900 to-blue-800 rounded-2xl overflow-hidden ring-1 ring-blue-900/60 text-sm">
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-blue-800/60 bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700">
            <div className="flex items-center gap-2">
              ...
              <p className="font-semibold text-cyan-200 text-lg drop-shadow">All Appointments</p>
            </div>
          ...
          <div className="max-h-[75vh] min-h-[50vh] overflow-y-auto">
            {(appointments || []).map((item, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <img className="w-9 h-9 rounded-full object-cover ring-2 ring-cyan-400/40 shadow-md" src={item?.userData?.image || ''} alt="Patient avatar" />
                  <p className="text-cyan-100 font-semibold">{item?.userData?.name || '—'}</p>
                </div>

                <div>
                  <span className={`text-xs px-2 py-0.5 rounded-full ring-1 font-bold ${item?.payment ? 'bg-green-900/60 text-green-300 ring-green-400/40' : 'bg-yellow-900/60 text-yellow-300 ring-yellow-400/40'}`}>
                    {item?.payment ? 'Online' : 'Cash'}
                  </span>
                </div>

                <p className="text-cyan-200">{calculateAge(item?.userData?.dob)}</p>
                <p className="text-cyan-200">{slotDateFormat(item?.slotDate)}, {item?.slotTime}</p>
                <p className="text-cyan-100 font-bold">{currency(item?.amount)}</p>
