...
import { DoctorContext } from '../../contexts/doctorContext';  // use doctor context to access dToken and backendUrl
...
//The component runs this effect when it first renders or when token or backendUrl changes.
	...
	useEffect(() => {
		...
		const fetchInbox = async () => {
			...
			try {
				setLoading(true);
                // Makes a GET request to the backend to fetch the doctor's inbox data, including the authentication token in the headers.
	...
	const openChat = (item) => {
		navigate(`/doctor/chat/${item.appointmentId}`, { state: { patientName: item.user?.name || 'Patient' } });  // navigate to chat route defined in admin App and pass patientName
	};
