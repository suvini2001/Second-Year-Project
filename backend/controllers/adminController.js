...
const addDoctor = async (req, res) => {
  try {
    ...
    if (!doctorData.experience && doctorData.experiance)
      doctorData.experience = doctorData.experiance;
    if (!doctorData.fees && doctorData.fee) doctorData.fees = doctorData.fee;
    if (!doctorData.specialization && doctorData.speciality)
      doctorData.specialization = doctorData.speciality;
    ...
    doctorData.experience = expRaw
      ? parseInt(expRaw.replace(/\D/g, ""), 10)
      : 0;
    ...
    if (doctorData.date) {
      const parsedDate = new Date(
        doctorData.date.toString().replace(/\./g, "-")
      );
...
const appointmentCancel = async (req, res) => {
  try {
    ...
    const appt =
      appointmentData && appointmentData.toObject
